...
function Homepage({ searchBarWidth }) {
  ...
  console.log("Search Results in the Redux store:", searchResults);
      ...
      {searchResults && Object.keys(searchResults).length > 0 && (
        <div className="chart-details-container" style={{ marginLeft: "20px" }}>
          <div className="chart-details-card" style={{ color: "black" }}>
            <Title> Details </Title>
            <p>Exchange: </p>
            <p>Price: {searchResults?.lastPrice}</p>
            <p>Status: {searchResults?.tradingStatus}</p>
            <p>
              Market Cap: {millify(searchResults?.marketCap)}{" "}
              {searchResults?.currencySymbol}
            </p>
          </div>
        </div>
      )}
